    ...
    IProjectManagementDisplayProjectionService? displayProjection = null) : IProjectManagementRecycleService, ITransientDependency
{
    ...
    private async Task<ProjectManagementRecyclePurgeImpact> GetPurgeImpactAsync(string projectId, IReadOnlyCollection<string>? taskIds, CancellationToken cancellationToken)
    {
        ...
        return new ProjectManagementRecyclePurgeImpact(allProjectData ? 1 : 0, taskCount, Math.Max(0, taskCount - 1), memberCount, milestoneCount, dependencyCount, participantCount, labelRelationCount, commentCount, attachmentCount, reminderCount, notificationCount, recurrenceCount, occurrenceCount, conversationLinkCount, savedViewCount, syncJournalCount);
    }
    ...
    private async Task<string?> GetTaskPurgeBlockingReasonAsync(string projectId, IReadOnlyCollection<string> taskIds, CancellationToken cancellationToken)
    {
        ...
        return hasOperationalReferences ? "任务仍存在参与人、标签、评论、附件、通知、提醒或排期等业务关联，必须先处理这些关联后才能永久删除" : null;
    }

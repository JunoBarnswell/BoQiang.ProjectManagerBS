...
public sealed class ProjectManagementSavedViewService(IWorkspaceDatabaseAccessor databaseAccessor, ICurrentUser currentUser, ProjectManagementAccessPolicy? accessPolicy = null) : IProjectManagementSavedViewService
{
    ...
    public async Task<IReadOnlyList<ProjectManagementSavedViewResponse>> QueryAsync(string projectId, CancellationToken cancellationToken = default)
    {
        ...
        var user = User();
        var rows = await databaseAccessor.GetProjectManagementDb().Queryable<ProjectManagementSavedViewEntity>().Where(item => item.ProjectId == projectId && !item.IsDeleted && (item.IsShared || item.OwnerUserId == user)).OrderBy(item => item.IsDefault, OrderByType.Desc).OrderBy(item => item.ViewName).ToListAsync(cancellationToken);
    ...
    private async Task EnsureProjectAsync(string projectId, CancellationToken cancellationToken) { Tenant(); App(); if (!await databaseAccessor.GetProjectManagementDb().Queryable<ProjectManagementProjectEntity>().Where(item => item.Id == projectId && !item.IsDeleted).AnyAsync(cancellationToken)) throw new NotFoundException("项目不存在", ErrorCodes.PlatformResourceNotFound); }

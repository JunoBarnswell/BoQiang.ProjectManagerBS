    ...
    IProjectManagementRealtimePublisher? realtimePublisher = null) : IProjectManagementTaskCommentService
{
    ...
    public async Task DeleteAsync(string taskId, string id, long versionNo, CancellationToken cancellationToken = default)
    {
        ...
        {
            await db.Updateable(entity).ExecuteCommandAsync(cancellationToken);
            await WriteActivityAsync(task, entity, "comment.deleted", "删除任务评论", cancellationToken);
        });
    ...
    private async Task WriteActivityAsync(ProjectManagementTaskEntity task, ProjectManagementTaskCommentEntity comment, string activityType, string summary, CancellationToken cancellationToken)
    {
        if (activityWriter is null) return;
        await activityWriter.AppendAsync(new ProjectManagementActivityEvent(Tenant(), App(), "TaskComment", comment.Id, activityType, summary, Activity.Current?.Id ?? Guid.NewGuid().ToString("N"), User(), task.ProjectId), cancellationToken);
    }

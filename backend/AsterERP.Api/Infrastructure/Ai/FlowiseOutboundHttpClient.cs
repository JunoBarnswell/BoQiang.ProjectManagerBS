...
internal static class FlowiseOutboundHttpClient
{
    public const string Name = "flowise-outbound";
    ...
    public static HttpMessageHandler CreatePrimaryHandler() =>
        new FlowiseOutboundHttpMessageHandler(CreateSocketsHandler());
    ...
    internal static void EnsureAllowedUri(Uri? uri)
    {
        ...
        {
            throw new ValidationException("Flowise 出站请求仅支持有效的 http/https 绝对地址", ErrorCodes.ParameterInvalid);
        }

        if (!string.IsNullOrWhiteSpace(uri.UserInfo))
        {
            throw new ValidationException("Flowise 出站请求地址禁止携带用户信息", ErrorCodes.ParameterInvalid);
        }
        ...
        {
            throw new ValidationException("Flowise 出站请求禁止访问本机地址", ErrorCodes.PermissionDenied);
        }

        if (IPAddress.TryParse(host, out var address) && !IsPublicAddress(address))
        {
            throw new ValidationException("Flowise 出站请求禁止访问私有或保留网络地址", ErrorCodes.PermissionDenied);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new ValidationException("Flowise 出站请求目标域名无法解析", ErrorCodes.ParameterInvalid);
        }
    ...
    internal static void EnsureResolvedAddressesArePublic(IReadOnlyCollection<IPAddress> addresses)
    {
        if (addresses.Count == 0 || addresses.Any(address => !IsPublicAddress(address)))
        {
            throw new ValidationException(
                "Flowise 出站请求目标解析到了私有、回环或保留网络地址",
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new HttpRequestException("Flowise 出站请求无法连接到已验证的公网地址", lastError);
    }
...
internal sealed class FlowiseOutboundHttpMessageHandler(HttpMessageHandler innerHandler)
    : DelegatingHandler(innerHandler)
{
    protected override async Task<HttpResponseMessage> SendAsync(
        ...
        CancellationToken cancellationToken)
    {
        FlowiseOutboundHttpClient.EnsureAllowedUri(request.RequestUri);
        ...
        {
            throw new ValidationException(
                "Flowise 出站请求禁止覆盖 Host 或代理认证头",
                ErrorCodes.PermissionDenied);
        }

        await FlowiseOutboundHttpClient.ResolvePublicAddressesAsync(
...
internal sealed class FlowiseScopedHttpClientFactory(IHttpClientFactory innerFactory)
    : IHttpClientFactory
{
    public HttpClient CreateClient(string name)
    {
        var resolvedName = string.IsNullOrEmpty(name)
            ? FlowiseOutboundHttpClient.Name
            : name;

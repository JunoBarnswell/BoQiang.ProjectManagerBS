...
public sealed class ProjectManagementTaskProgressProjectorTests
{
    ...
    private static async Task SeedProjectAsync(ISqlSugarClient db)
    {
        ...
        await db.Insertable(new[]
        {
            ...
            new ProjectManagementTaskEntity { Id = "leaf-done", TenantId = "tenant-a", AppCode = "MES", ProjectId = "project-a", ParentTaskId = "middle", MilestoneId = "milestone-a", TaskCode = "DONE", Title = "Done", Status = ProjectManagementDomainRules.TaskDone, ProgressPercent = 100m, Weight = 30, AssigneeUserId = "alice", DueDate = overdue },
            new ProjectManagementTaskEntity { Id = "leaf-blocked", TenantId = "tenant-a", AppCode = "MES", ProjectId = "project-a", ParentTaskId = "middle", MilestoneId = "milestone-a", TaskCode = "BLOCKED", Title = "Blocked", Status = ProjectManagementDomainRules.TaskBlocked, ProgressPercent = 40m, Weight = 90, AssigneeUserId = "bob", DueDate = overdue },
            ...
            new ProjectManagementTaskEntity { Id = "cancelled-child", TenantId = "tenant-a", AppCode = "MES", ProjectId = "project-a", ParentTaskId = "empty-parent", TaskCode = "CANCELLED", Title = "Cancelled", Status = ProjectManagementDomainRules.TaskCancelled, ProgressPercent = 100m },
            new ProjectManagementTaskEntity { Id = "deleted-leaf", TenantId = "tenant-a", AppCode = "MES", ProjectId = "project-a", TaskCode = "DELETED", Title = "Deleted", Status = ProjectManagementDomainRules.TaskDone, ProgressPercent = 100m, IsDeleted = true }
        }).ExecuteCommandAsync();
    }

...
public sealed class ProjectManagementSchemaMigratorTests
{
    ...
    public async Task Migration_is_idempotent_and_creates_versioned_pm_schema()
    {
        ...
        Assert.Equal(6, db.Ado.GetInt("SELECT VersionNo FROM pm_schema_versions WHERE ModuleKey = 'project-management'"));
        ...
        Assert.Equal(0, db.Ado.GetInt("SELECT COUNT(1) FROM sqlite_master WHERE type = 'table' AND name = 'pm_task_time_logs'"));
        Assert.Equal(0, db.Ado.GetInt("SELECT COUNT(1) FROM pragma_table_info('pm_tasks') WHERE name IN ('EstimateMinutes', 'ActualMinutes')"));
    }

    ...
    public async Task<object?> ExecuteAsync(ICommandContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (job.JobType != AbstractJobEntity.JobTypeTimer)
            throw new WorkflowEngineObjectNotFoundException(
                $"No timer job found with id '{_jobId}', actual type is '{job.JobType}'.",
                typeof(JobEntity));

        await jobManager.SetJobRetriesAsync(_jobId, _retries, cancellationToken);

        JobCommandEvents.DispatchEntityEvent(context, WorkflowEventType.ENTITY_UPDATED, new { JobId = _jobId, Retries = _retries, JobCategory = "timer" });
        return null;
    }
}

...
public class AcquireJobsCmd : ICommand<AcquiredJobs>
{
    private readonly int _maxJobsPerAcquisition;
    private readonly string _lockOwner;
    private readonly int _lockTimeInMillis;

    public AcquireJobsCmd(int maxJobsPerAcquisition, string lockOwner, int lockTimeInMillis)
    {
    ...
    public async Task<AcquiredJobs> ExecuteAsync(ICommandContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (context.ProcessEngineConfiguration.JobManager is not IJobLifecycleManager lifecycleManager)
            throw new WorkflowEngineException("Current job manager does not support lifecycle operations for acquiring executable jobs.");
        ...
        var jobIds = jobs.Select(job => job.Id).Where(id => !string.IsNullOrWhiteSpace(id)).ToList();
        if (jobIds.Count > 0)

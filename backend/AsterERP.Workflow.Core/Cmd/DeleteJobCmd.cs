    ...
    public async Task<object?> ExecuteAsync(ICommandContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (job.JobType is AbstractJobEntity.JobTypeTimer or AbstractJobEntity.JobTypeDeadLetter)
            throw new WorkflowEngineObjectNotFoundException(
                $"No executable job found with id '{_jobId}', actual type is '{job.JobType}'.",
                typeof(JobEntity));

        await jobManager.DeleteJobAsync(_jobId, cancellationToken);

        JobCommandEvents.DispatchEntityEvent(context, WorkflowEventType.JOB_CANCELED, new { JobId = _jobId });
        return null;
    }
}

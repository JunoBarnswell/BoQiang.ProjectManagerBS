...
- `Modules/ProjectManagement` / `Application/ProjectManagement` / `Contracts/ProjectManagement`
  - 项目管理原生业务域，按 `项目 -> 里程碑 -> 主题父任务 -> Sub-issues` 组织项目执行数据。
  - 当前 M0 已建立 ABP 模块、模块契约、权限码和发布裁剪边界；项目、任务、视图和协作业务将在后续里程碑逐 Case 接入。
...
| 2026-07-18 | 项目管理 | ProjectManagement ABP module（SYSTEM 平台 `platform -> project-management` 菜单） | project-management:project:* / project-management:member:* / project-management:milestone:* / project-management:task:* / project-management:label:view/manage / project-management:comment:* / project-management:attachment:manage / project-management:notification:view / project-management:reminder:view/manage / project-management:im-conversation:view/manage / project-management:report:* / project-management:sync:* / project-management:backup:* / project-management:audit:* | 项目管理权限和固定菜单仅由 SYSTEM 平台主库播种并授权平台 `admin`；应用 shell baseline 会退休历史 `project-management` 菜单和权限，应用 `app_admin` 不自动获得项目管理权限。工作台已统一工具栏（树状/平铺/卡片/看板/甘特/日历 + 搜索/筛选/刷新/通知入口），看板与表格支持状态流转校验，任务编辑器分详情/评论/附件/提醒/工时/活动页签；工时走 task:view/edit，提醒走 reminder:view/manage，附件走 attachment:manage；顶栏通知与浏览器通知覆盖提醒/提及/指派等高价值类型 | 项目管理服务固定使用平台主库；项目域实体保留 TenantId + AppCode，并在项目对象上叠加 ProjectId；列表、详情、统计、导出、批量、关联会话和附件统一走 ORM data filter |
| 2026-07-20 | 项目管理工作台 UX | ProjectRequirementsScreen / ProjectWorkItemEditor / ProjectManagementTaskTimeLogsController / ProjectManagementTaskRemindersController | project-management:task:view/edit/add / project-management:reminder:view/manage / project-management:attachment:manage / project-management:notification:view | 已收录；前端工作台已接线日历/甘特/卡片视图、状态流转、进度条、提醒与工时 UI，权限双侧校验 | 工时与提醒读写均在任务与项目 ORM data filter 边界内；无新增旁路数据权限 |
| 2026-07-20 | 项目任务外部 @ 提及共享 | ProjectManagementTaskCommentsController / ProjectManagementTaskCommentService / ProjectManagementNotificationService | project-management:comment:view/add / project-management:task:share / project-management:task:view / project-management:notification:view | 已收录；仅拥有 `task:share` 且具备目标任务管理权的用户可通过评论 @ 向项目外人员授予任务级阅读，`AllowMentionedComment` 仅决定该授权能否新增评论；所有任务编辑、成员、附件、工时与提醒动作仍由原权限码和对象级策略拒绝。被授权人还须被分配项目查看、任务查看、通知查看，以及需要评论时的评论新增基础权限。 | `pm_task_grants` 被纳入 ORM data filter：被授权人仅可见对应任务及其评论/附件等任务级关联数据，项目过滤只为导航保留项目可见性；任务写权限没有因授权放开。通知经持久化 + SignalR + `/open` 二次鉴权后深链到评论区。 |
| 2026-07-20 | 项目成员配置 | ProjectManagementProjectsController / ProjectManagementMemberService / 项目工作台成员配置 | project-management:project:add / project-management:member:view/manage | 已收录；创建项目可原子写入负责人之外的 Manager、Lead、Member、Viewer 初始成员，负责人始终由项目负责人字段创建为唯一 Owner；成员配置入口位于项目工作台，Lead 的主题根任务范围仅可在项目创建后维护。`member:view` 控制成员列表，`member:manage` 控制新增、角色/范围修改和移除，后端仍执行对象级授权。 | 初始成员写入与项目、Owner 成员位于同一事务；候选用户必须处于当前租户且启用。成员、项目、任务及 Lead 范围查询持续由项目管理 ORM data filter 约束，不接受前端提交的数据范围作为授权条件。 |
| 2026-07-20 | 项目跟踪体验升级 | ProjectManagementReportsController / ProjectManagementReportService / ProjectOverviewScreen / ProjectSelectorScreen | project-management:report:export（复用）/ project-management:task:add（快捷新建） | 已收录；单项目 Markdown 汇报导出复用既有报表导出权限，概览/项目卡片展示倒计时和进展入口 | Markdown 导出只从当前工作区已注册 ORM data filter 范围内读取项目、任务、评论和活动数据；无新增数据权限旁路 |
| 2026-07-19 | 项目管理审计治理 | ProjectManagementAuditController / ProjectManagementAuditGovernanceService | project-management:operation:manage | 已收录；保留策略由 ABP 设置提供，治理清理通过 `audit.governance.cleanup` 后台操作执行，策略、操作者、结果数量、容量告警和失败状态写入 `pm_operations` | 一般活动在 `pm_activities` 内归档并继续沿用 ORM 数据权限；高风险活动和 `pm_operations` 永不由治理清理删除；详见 `docs/project-management-audit-governance.md` |
...
| 日期 | 变更人 | 内容 |
| --- | --- | --- |
| 2026-07-18 | Codex | 建立 ProjectManagement ABP 模块边界，接入宿主模块、module-file-map、项目管理权限码和菜单功能清单；业务功能按 M0-M7 逐 Case 实施并在测试证据完成后关闭 Linear 状态。 |
| 2026-07-20 | Codex | 项目管理围绕跟踪心智升级：新增受既有报表权限保护的单项目 Markdown 汇报导出（项目、里程碑、任务、评论与动态时间线），并在项目概览/列表/任务编辑器加入倒计时、进度 hero 与更聚焦的时间线入口；所有项目数据仍经 ORM data filter 读取。 |

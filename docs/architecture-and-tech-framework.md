...
| 2026-07-18 | 项目管理 | ProjectManagement ABP module（SYSTEM 平台 `platform -> project-management` 菜单） | project-management:project:* / project-management:member:* / project-management:milestone:* / project-management:task:* / project-management:comment:* / project-management:notification:view / project-management:reminder:view/manage / project-management:im-conversation:view/manage / project-management:report:* / project-management:sync:* / project-management:backup:* / project-management:audit:* | 项目管理权限和固定菜单仅由 SYSTEM 平台主库播种并授权平台 `admin`；应用 shell baseline 会退休历史 `project-management` 菜单和权限，应用 `app_admin` 不自动获得项目管理权限。项目任务详情已接入提醒区；顶栏通知通过受权限保护的通知查询、已读和安全跳转接口进入任务工作区；项目/任务可创建并安全打开关联 IM 群会话，成员变化、归档与恢复由项目域服务统一编排；通知写入后按 tenant/app/user 实时刷新列表，浏览器通知仅在用户主动授权、后台页面和未展示通知时触发，点击仍走后端重新鉴权 | 项目管理服务固定使用平台主库；项目域实体保留 TenantId + AppCode，并在项目对象上叠加 ProjectId；列表、详情、统计、导出、批量、关联会话和附件统一走 ORM data filter |

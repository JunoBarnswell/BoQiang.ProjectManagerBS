...
| 2026-07-18 | 项目管理 | ProjectManagement ABP module（业务接口将在 M1-M4 逐步接入） | project-management:project:* / project-management:member:* / project-management:milestone:* / project-management:task:* / project-management:comment:* / project-management:notification:view / project-management:reminder:view/manage / project-management:im-conversation:view/manage / project-management:report:* / project-management:sync:* / project-management:backup:* / project-management:audit:* | 项目任务详情已接入提醒区；顶栏通知通过受权限保护的通知查询、已读和安全跳转接口进入任务工作区；项目/任务可创建并安全打开关联 IM 群会话，成员变化、归档与恢复由项目域服务统一编排；通知写入后按 tenant/app/user 实时刷新列表，浏览器通知仅在用户主动授权、后台页面和未展示通知时触发，点击仍走后端重新鉴权；项目 SignalR 以 tenant/app/project 授权分组，事件仅下发对象标识、版本和变更类型，前端按版本水位合并失效并在重连后重新对账；后端以提醒记录为状态源，Hangfire 仅持有工作区与提醒标识 | 所有项目域实体必须使用 TenantId + AppCode，并在项目对象上叠加 ProjectId；列表、详情、统计、导出、批量、关联会话和附件统一走 ORM data filter |

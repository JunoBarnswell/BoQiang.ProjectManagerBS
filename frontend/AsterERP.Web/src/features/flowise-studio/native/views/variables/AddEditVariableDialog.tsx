...
export function AddEditVariableDialog({ item, open, saving, onClose, onSubmit }: AddEditVariableDialogProps) {
  const { translate } = useI18n();
  const [draft, setDraft] = useState<FlowiseResourceUpsertRequest>(() => createResourceDraft());

  useEffect(() => {
    if (open) {
      setDraft(item ? toResourceDraft(item) : createResourceDraft({ category: 'runtime', metadataJson: '{"secret":false}' }));
    }
  }, [item, open]);

  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      <DialogTitle>{item ? 'Edit Variable' : 'Add Variable'}</DialogTitle>
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <TextField label="Variable Type" value={draft.category ?? ''} onChange={(event) => setDraft({ ...draft, category: event.target.value })} />
          <TextField label="Value / Secret" type="password" value={draft.secretValue ?? ''} onChange={(event) => setDraft({ ...draft, secretValue: event.target.value })} />
          ...
          <FormControl>
            <InputLabel id="variable-status-label">{translate(flowiseI18nKeys.fields.status)}</InputLabel>
            <Select label={translate(flowiseI18nKeys.fields.status)} labelId="variable-status-label" value={draft.status ?? 'Enabled'} onChange={(event) => setDraft({ ...draft, status: event.target.value })}>

...
export function ImChatPanel({ conversation, readonly, sourceAppCode }: ImChatPanelProps) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-white">
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-gray-200 px-4">
        <div className="truncate text-sm font-semibold text-gray-900">{conversation.peerDisplayName}</div>
      </div>
      <div className="min-h-0 flex-1 overflow-auto bg-gray-50 p-4">
        ...
        <div className="space-y-3">
          {messages.map((message) => {
            const mine = message.senderUserId !== conversation.peerUserId;
            return (

...
import { PmBox, PmChip, PmStack, PmText, PmTooltip } from '../../../../ui/project-management';
...
export function DependencyAnalysisOverlay({ analysis, rows, width, height, onSelectTask }: DependencyAnalysisOverlayProps) {
  ...
  return (
    <PmBox sx={{ inset: 0, pointerEvents: 'none', position: 'absolute' }}>
      ...
      <PmStack direction="row" spacing={0.75} sx={{ left: 8, pointerEvents: 'auto', position: 'absolute', top: 8 }}>
        <PmChip color="error" label={`关键任务 ${criticalByTaskId.size === 0 ? 0 : [...criticalByTaskId.values()].filter(Boolean).length}`} />
        {analysis.diagnostics.length > 0 && (
          <PmTooltip title={analysis.diagnostics.map((item) => item.message).join('\n')}>
            <PmChip color="warning" label={`依赖异常 ${analysis.diagnostics.length}`} />
          </PmTooltip>
        )}
      </PmStack>
      {analysis.diagnostics.length > 0 && (
        <PmBox sx={{ bottom: 8, maxWidth: 440, pointerEvents: 'auto', position: 'absolute', right: 8 }}>

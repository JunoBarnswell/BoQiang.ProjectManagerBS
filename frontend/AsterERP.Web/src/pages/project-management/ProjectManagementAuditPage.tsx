...
export function ProjectManagementAuditPage() {
  ...
  const query: ProjectManagementAuditQuery = { pageIndex: 1, pageSize: 100, keyword: submittedKeyword || undefined };
  ...
  return (
    <ResponsivePage
      ...
      toolbar={<PermissionButton code="project-management:audit:export" disabled={exportMutation.isPending} onClick={() => exportMutation.mutate()}>{exportMutation.isPending ? '导出中…' : '导出 CSV'}</PermissionButton>}
    >
      <div className="mb-3 flex items-center gap-2">
        <form className="flex items-center gap-2" onSubmit={(event) => { event.preventDefault(); setSubmittedKeyword(keyword.trim()); }}><input className="rounded border border-gray-300 px-3 py-2" aria-label="审计关键字" placeholder="搜索摘要、操作者或对象" value={keyword} onChange={(event) => setKeyword(event.target.value)} /><button type="submit">搜索</button>{submittedKeyword ? <button type="button" onClick={() => { setKeyword(''); setSubmittedKeyword(''); }}>清空</button> : null}</form>

...
export function ProjectManagementAuditPage() {
  ...
  const [exportFields, setExportFields] = useState<ProjectManagementAuditExportField[]>(['Project', 'Object', 'ActivityType', 'Summary', 'TraceId', 'Actor', 'CreatedTime']);
    ...
    >
      <div className="mb-3 flex items-center gap-2">
        <form className="flex flex-wrap items-center gap-2" onSubmit={(event) => { event.preventDefault(); setPageIndex(1); setSubmittedFilters(filters); }}>
          ...
          <input className="rounded border border-gray-300 px-3 py-2" aria-label="项目名称" placeholder="项目名称" value={filters.projectId} onChange={(event) => setFilters((current) => ({ ...current, projectId: event.target.value }))} />
          ...
          <input className="rounded border border-gray-300 px-3 py-2" aria-label="操作者" placeholder="操作者名称" value={filters.actorUserId} onChange={(event) => setFilters((current) => ({ ...current, actorUserId: event.target.value }))} />
       ...
       <div className="mb-3 flex flex-wrap items-center gap-3 rounded border border-gray-200 p-3 text-sm">
         ...
         {(['Project', 'Object', 'ActivityType', 'Summary', 'TraceId', 'Actor', 'CreatedTime', 'Source', 'FieldChanges'] as ProjectManagementAuditExportField[]).map((field) => <label key={field} className="inline-flex items-center gap-1"><input type="checkbox" checked={exportFields.includes(field)} onChange={(event) => setExportFields((current) => event.target.checked ? [...current, field] : current.filter((item) => item !== field))} />{({ Project: '项目', Object: '对象', Actor: '操作者' } as Record<string, string>)[field] ?? field}</label>)}
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>{page.items.length === 0 ? <tr><td className="px-3 py-6 text-center text-gray-500" colSpan={10}>{hasAuditFilters(submittedFilters) ? '没有匹配的审计记录' : '暂无审计记录'}</td></tr> : page.items.map((item) => <tr className="border-t border-gray-100" key={item.id}><td className="whitespace-nowrap px-3 py-2">{new Date(item.createdTime).toLocaleString()}</td><td className="px-3 py-2">{item.projectDisplayName ?? '项目已删除或无权查看'}</td><td className="px-3 py-2">{item.aggregateDisplayName ?? '对象已删除或无权查看'}</td><td className="px-3 py-2">{item.activityType}</td><td className="px-3 py-2">{item.source}{item.sourceDeviceId ? ` / ${item.sourceDeviceId}` : ''}</td><td className="px-3 py-2">{item.isSuccess ? '成功' : '失败'}</td><td className="max-w-md px-3 py-2">{item.summary ?? '-'}</td><td className="px-3 py-2">{item.actorDisplayName ?? '用户已删除或无权查看'}</td><td className="px-3 py-2 font-mono text-xs">{item.traceId}</td><td className="px-3 py-2"><button className="underline" type="button" onClick={() => setSelectedAuditId(item.id)}>查看</button></td></tr>)}</tbody>
        </table>
      </div>

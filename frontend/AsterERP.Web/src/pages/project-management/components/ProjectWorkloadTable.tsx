...
export function ProjectWorkloadTable({ displayNames, isError, isLoading, onRetry, rows }: ProjectWorkloadTableProps) {
  ...
  return <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr><th className="px-2 py-2 text-left">人员</th><th className="px-2 py-2 text-right">待办</th><th className="px-2 py-2 text-right">进行中</th><th className="px-2 py-2 text-right">已完成</th><th className="px-2 py-2 text-right">逾期</th><th className="px-2 py-2 text-right">预计</th><th className="px-2 py-2 text-right">已登记</th></tr></thead><tbody>{workloadRows.map((person) => <tr className="border-t border-gray-100" key={person.userId}><td className="px-2 py-2 font-medium">{person.displayName || displayNames[person.userId] || '暂无别名'}</td><td className="px-2 py-2 text-right">{person.todoTaskCount}</td><td className="px-2 py-2 text-right">{person.inProgressTaskCount}</td><td className="px-2 py-2 text-right">{person.completedTaskCount}</td><td className="px-2 py-2 text-right">{person.overdueTaskCount}</td><td className="px-2 py-2 text-right">{formatMinutes(person.estimatedMinutes)}</td><td className="px-2 py-2 text-right">{formatMinutes(person.loggedMinutes)}</td></tr>)}</tbody></table></div>;
}

...
export function ProjectManagementOverviewPage() {
  ...
  return (
    <ResponsivePage
      ...
      toolbar={<div className="flex flex-col gap-2"><div className="pm-toolbar-summary"><span>{project.projectCode}</span><ProjectStatus status={project.status} /><span>负责人：{project.ownerUserId || '未分配'}</span></div><nav aria-label="当前项目导航" className="flex flex-wrap gap-2 text-sm">{canViewProject ? <Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/overview`)}>概览</Link> : null}{canViewTasks ? <><Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/tasks`)}>任务树</Link><Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/settings`)}>设置</Link></> : null}{canViewMilestones ? <Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/milestones`)}>里程碑</Link> : null}{canViewMembers ? <Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/members`)}>成员</Link> : null}{canExportReports ? <Link className="rounded border border-gray-300 px-3 py-1" to={toProjectManagementPlatformRoute(`projects/${encodeURIComponent(projectId)}/reports`)}>报表</Link> : null}</nav></div>}
    >

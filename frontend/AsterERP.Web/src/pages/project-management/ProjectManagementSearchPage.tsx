...
export function ProjectManagementSearchPage() {
  ...
  const query = useMemo(() => ({ keyword: submittedKeyword, scope: searchScope, limit: 50 }), [searchScope, submittedKeyword]);
  ...
  const total = (result?.projects.length ?? 0) + (result?.tasks.length ?? 0) + (result?.comments.length ?? 0);
  return (
    <ResponsivePage
      title="项目域搜索"
      ...
      toolbar={
        <form className="flex flex-wrap items-center gap-2" onSubmit={(event) => { event.preventDefault(); setSubmittedKeyword(keyword.trim()); }}>
          <input aria-label="搜索关键字" className="min-w-64" maxLength={200} placeholder="搜索项目、任务或评论" value={keyword} onChange={(event) => setKeyword(event.target.value)} />
...
function SearchEmptyState() {
  return <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center text-sm text-gray-500">输入至少一个关键字后开始搜索。项目、任务和评论都会由服务端在当前授权范围内检索。</div>;
}
...
function SearchResultGroup({ emptyText, items, onOpen, title }: { emptyText: string; items: ProjectManagementSearchItem[]; onOpen: (item: ProjectManagementSearchItem) => void; title: string }) {
  return <section aria-label={title}>
    ...
    {items.length === 0 ? <div className="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-500">{emptyText}</div> : <div className="overflow-x-auto rounded-lg border border-gray-200"><table className="min-w-full text-left text-sm"><thead className="bg-gray-50"><tr><th className="px-3 py-2">标题</th><th className="px-3 py-2">摘要</th><th className="px-3 py-2">最近更新</th><th className="px-3 py-2">操作</th></tr></thead><tbody>{items.map((item) => <tr className="border-t border-gray-100" key={`${item.resultType}-${item.id}`}><td className="px-3 py-2 font-medium">{item.title}</td><td className="max-w-xl px-3 py-2">{item.summary ?? '-'}</td><td className="whitespace-nowrap px-3 py-2">{item.updatedTime ? new Date(item.updatedTime).toLocaleString() : '-'}</td><td className="px-3 py-2"><button type="button" onClick={() => onOpen(item)}>打开</button></td></tr>)}</tbody></table></div>}
  </section>;
}
